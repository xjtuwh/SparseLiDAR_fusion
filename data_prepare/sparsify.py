...
def pto_ang_map(velo_points, H=64, W=512, slice=1, line_spec=None,
    ...
    phi = np.radians(180.) - np.arctan2(y,x)
    ...
    start = round(((180-70)/0.09))
    end = round(((360-110)/0.09))
    sample_num = np.arange(start,end,3)
    if line_spec is not None:
        depth_map = depth_map[line_spec, :, :]
        depth_map = depth_map[:,sample_num,:]
    else:
