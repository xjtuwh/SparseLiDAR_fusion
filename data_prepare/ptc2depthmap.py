...
def get_depth_map(ptc, calib, img):
    ...
    fov_inds = (ptc_image[:, 0] < width - 1) & (ptc_image[:, 0] >= 0) & \
               (ptc_image[:, 1] < height - 1) & (ptc_image[:, 1] >= 0)
    fov_inds = fov_inds & (ptc[:, 0] > 2)
    ptc = ptc[fov_inds, :]
    ptc_image = ptc_image[fov_inds, :]
...
def convert_and_save(args, i):
    ...
    cv2.imwrite(osp.join(args.output_path, "{:06d}.png".format(i)), depth_map)
