...
# from lib.losses.disparity_loss import DisparityLoss

def compute_centernet3d_loss(input, target):
    stats_dict = {}
    target['mask_2d'] = target['mask_2d'].bool()
    target['mask_3d'] = target['mask_3d'].bool()
    # disparity_loss = compute_disparity_loss(input['disp_out'],target['dep_map'])
    ...
    # stats_dict['disp'] = disparity_loss.item()

    total_loss = seg_loss + offset2d_loss + size2d_loss + offset3d_loss + \
                 size3d_loss + heading_loss
    return total_loss, depth_loss, stats_dict

# def compute_disparity_loss(input, target):
#     # target = target.unsqueeze(1)
#     disparity_loss = DisparityLoss(maxdisp=96)
#     return disparity_loss(input,target)
...
def compute_depth_loss(input, target):
    ...
    batch, K,_ = target['depth'].size()
    depth_input = input['depth'].view(batch,K,2)[target['mask_3d']]
    # depth_input= depth_input[:, 0:1]
    ...
    # depth_loss = F.l1_loss(depth_input, depth_target, reduction='sum') / max(1, depth_input.size(0))
    return depth_loss
...
def compute_offset3d_loss(input, target):
    ...
    offset3d_loss = F.l1_loss(offset3d_input, offset3d_target, reduction='sum')/max(1,offset3d_input.size(0)*offset3d_input.size(1))
...
def compute_size3d_loss(input, target):
    ...
    size3d_loss = F.l1_loss(size3d_input, size3d_target,reduction='sum')/max(1,size3d_input.size(0)*size3d_input.size(1))
    return size3d_loss
...
def compute_heading_loss(input, target):
    ...
    reg_loss = F.l1_loss(heading_input_res, heading_target_res, reduction='sum')/max(1,mask.sum())
